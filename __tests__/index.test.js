...
test('genDiff / nested / stylish', () => {
    expect(genDiff(getFixturePath('nestedFile1.json'), getFixturePath('nestedFile2.json'), 'stylish')).toEqual(resultNestedStylish);
});
...
test('genDiff / Error / Unknown file extension!', () => {
    expect(() => genDiff(getFixturePath('file.js'), getFixturePath('file.js'))).toThrow(new Error('Unknown file extension!'));
});

test('genDiff / Error / Wrong format!', () => {
    expect(() => genDiff(getFixturePath('simpleFile1.json'), getFixturePath('simpleFile2.json'), 'stuff')).toThrow(new Error('Wrong format!'));
});

test('genDiff / Error / Files are empty!', () => {
    expect(() => genDiff(getFixturePath('emptyFile1.json'), getFixturePath('emptyFile2.json'))).toThrow(new Error('Files are empty!'));
});
